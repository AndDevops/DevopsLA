...
while possible_divisor <= num:
    ...
    # of number % possible_divisor is 0 it means possible_divisor
    # is an actual divisor of the number
    if num % possible_divisor==0:
        s+=str(possible_divisor)+","
    possible_divisor+=1
# Remove suffix only removes if the string is ending with given charachers.
print(s.removesuffix(","),"are the divisors of the numbers.")
